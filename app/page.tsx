...
export default function Home() {
  return <main className="home-page"><HomeHeader />
    ...
    <section id="portfolio" className="section portfolio-section"><div className="shell"><div className="section-heading"><div><p className="eyebrow">Shopify Concepts</p><h2>Stores, die man<br />gern entdecken will.</h2></div><p className="portfolio-note">Unabhängige Konzeptstudien für Marken mit Anspruch.</p></div><div className="shopify-concept-grid">{shopifyConcepts.map((concept) => <article className="shopify-concept-card" key={concept.name}><div className="shopify-concept-preview"><Image src={concept.image} alt="" fill unoptimized loading="eager" sizes="(max-width: 760px) 100vw, 50vw" /></div><div className="shopify-concept-meta"><div><p>{concept.category}</p><h3>{concept.name}</h3><span>Shopify Concept</span></div><Link href="/portfolio" className="shopify-concept-link">Case Study <span aria-hidden="true">↗</span></Link></div></article>)}</div></div></section>
